function aoc_day10(part, input, findLow, findHigh) {
    ...
    while (true) {
        for (row of input) {
            if (row[0] === "b") {
                let [, from, lowToType, lowTo, highToType, highTo] =
                    row.match(/bot (\d+) gives low to (bot|output) (\d+) and high to (bot|output) (\d+)/);
                let bot = bots[from];
...
console.log(aoc_day10(1, instructions, 2, 5));

console.log(aoc_day10(2, instructions));
